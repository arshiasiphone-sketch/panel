...
export function getBlockDef(type: BlockType | string): BlockDef {
  ...
  return (
    BLOCK_DEFS.find((b) => b.type === normalized) ??
    BLOCK_DEFS.find((b) => b.type === type)! ??
    BLOCK_DEFS[0]
  );
}
...
export function defaultBlockData(type: BlockType | string): Record<string, any> {
  switch (normalizeBlockType(type)) {
    case "header":
      return { title: "عنوان جدید", subtitle: "" };
    case "paragraph":
      return { text: "متن خود را اینجا بنویسید..." };
    case "button":
      return { label: "کلیک کنید", url: "https://", style: "primary" };
    case "image":
      return { url: "", caption: "" };
    case "video":
      return { url: "https://www.youtube.com/embed/dQw4w9WgXcQ", title: "" };
    case "gallery":
      return { images: [] as string[] };
    case "menu":
      return { title: "منوی ما", items: [{ name: "اسپرسو", price: "۸۵٬۰۰۰" }] };
    case "instagram":
      return { handle: "@cafekhane" };
    case "booking":
      return { title: "رزرو میز", ctaLabel: "رزرو کنید" };
    case "contact":
      return { phone: "", email: "", whatsapp: "" };
    case "faq":
      return { items: [{ q: "ساعات کاری؟", a: "۱۰ صبح تا ۱۲ شب" }] };
    case "map":
      return { lat: 35.7, lng: 51.4, label: "کافه خانه" };
    case "divider":
      return { style: "line" };
    case "quote":
      return { text: "بهترین کافه شهر", author: "مهمان" };
    case "test":
      return { title: "تست شخصیت کافی", ctaLabel: "شروع تست" };
    case "event":
      return { title: "رویداد", date: "", description: "" };
    case "countdown":
      return { title: "شمارش معکوس", target: new Date(Date.now() + 7 * 86400000).toISOString() };
    case "social":
      return { links: [{ platform: "instagram", url: "" }] };
    case "file":
      return { label: "دانلود فایل", url: "", size: "" };
    case "music":
      return { provider: "spotify", url: "" };
    /* Landing sections */
    case "hero":
      return {
        badge: "کافه خانه",
    ...
    case "how_it_works":
      return { kicker: "چطور کار می‌کنه", title: "سه قدم ساده تا نتیجه" };
    ...
    case "parallax_gallery":
      return { kicker: "گالری", title: "لحظه‌های کافه خانه", subtitle: "" };
    case "gallery_preview":
      return { kicker: "گالری", title: "فضای کافه خانه", subtitle: "", count: 6, columns: 6 };
    case "events_preview":
      return { kicker: "رویدادها", title: "برنامه‌های ویژه", subtitle: "", count: 2 };
    case "testimonials_section":
      return { kicker: "تجربه‌ی دیگران", title: "چی می‌گن درباره‌اش" };
    case "location":
      return {
        kicker: "بیا پیش ما",
    ...
    case "stats":
      return {
        items: [
          { value: "۱۱", label: "سوال هوشمند" },
          { value: "۴", label: "تیپ شخصیتی" },
          { value: "۳", label: "دقیقه زمان" },
        ],
      };
    case "rich_text":
      return { title: "", text: "متن خود را اینجا بنویسید..." };
    case "custom_html":
      return { html: "<p>HTML سفارشی</p>" };
    case "spacer":
      return { height: 60 };
    default:
      return {};
...
export function BlockEditor({
  ...
  switch (normalizeBlockType(block.type)) {
    ...
    case "paragraph":
      return <TextArea label="متن" value={d.text ?? ""} onChange={(v) => update({ text: v })} />;
    case "quote":
      return (
        <div className="grid gap-3">
          <TextArea
    ...
    case "image":
      return (
        <div className="grid gap-3">
          ...
          <TextInput
            label="آدرس تصویر (یا به‌صورت دستی)"
            value={d.url ?? ""}
            onChange={(v) => update({ url: v })}
            dir="ltr"
            placeholder="https://..."
            ...
            render={(it, set) => (
              <div className="grid grid-cols-2 gap-2">
                ...
                <input
                  value={it.price ?? ""}
                  placeholder="قیمت"
                  onChange={(e) => set({ ...it, price: e.target.value })}
                  className={fieldClass()}
                />
              </div>
            )}
          ...
          render={(it, set) => (
            <div className="grid gap-2">
              <input
                value={it.q ?? ""}
                placeholder="سوال"
                onChange={(e) => set({ ...it, q: e.target.value })}
                className={fieldClass()}
              />
              <textarea
                value={it.a ?? ""}
                placeholder="پاسخ"
                onChange={(e) => set({ ...it, a: e.target.value })}
                rows={2}
                className={fieldClass()}
              />
            </div>
    ...
    case "map":
      return (
        <div className="grid gap-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className={labelClass()}>عرض</label>
              <input
                dir="ltr"
                type="number"
                value={d.lat ?? 0}
            ...
            <div>
              <label className={labelClass()}>طول</label>
              <input
                dir="ltr"
                type="number"
                value={d.lng ?? 0}
    ...
    case "countdown":
      return (
        <div className="grid gap-3">
          <TextInput label="عنوان" value={d.title ?? ""} onChange={(v) => update({ title: v })} />
          <div>
            <label className={labelClass()}>تاریخ هدف</label>
            <input
              type="datetime-local"
          ...
          render={(it, set) => (
            <div className="grid grid-cols-[110px_1fr] gap-2">
              <select
                ...
                onChange={(e) => set({ ...it, platform: e.target.value })}
                className={fieldClass()}
              >
                {[
                  "instagram",
              ...
              <input
                ...
                dir="ltr"
                placeholder="https://..."
                onChange={(e) => set({ ...it, url: e.target.value })}
                className={fieldClass()}
              />
            </div>
          )}
            ...
            render={(it, set) => (
              <div className="grid grid-cols-[1fr_1fr_70px] gap-2">
                <input
                  value={it.label ?? ""}
                  placeholder="نام"
                  onChange={(e) => set({ ...it, label: e.target.value })}
                  className={fieldClass()}
                />
                <input
                  value={it.tagline ?? ""}
                  placeholder="توضیح کوتاه"
                  onChange={(e) => set({ ...it, tagline: e.target.value })}
                  className={fieldClass()}
                />
                <input
                  type="color"
                  value={it.color ?? "#d4af37"}
                  onChange={(e) => set({ ...it, color: e.target.value })}
                  className={fieldClass()}
                />
              </div>
            )}
    ...
    case "how_it_works":
      return <SectionHeadingFields d={d} update={update} />;
    case "menu_highlights":
      return (
        <div className="grid gap-3">
          <SectionHeadingFields d={d} update={update} />
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className={labelClass()}>تعداد آیتم‌ها</label>
              <input
                type="number"
                min={1}
                max={12}
    ...
    case "gallery_preview":
      return (
        <div className="grid gap-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className={labelClass()}>تعداد تصاویر</label>
              <input
                type="number"
                min={1}
                max={24}
            ...
            <div>
              <label className={labelClass()}>تعداد ستون‌ها</label>
              <select
                value={d.columns ?? 6}
    ...
    case "events_preview":
      return (
        <div className="grid gap-3">
          <SectionHeadingFields d={d} update={update} />
          <div>
            <label className={labelClass()}>تعداد رویدادها</label>
            <input
              type="number"
              min={1}
              max={6}
          ...
          render={(it, set) => (
            <div className="grid grid-cols-2 gap-2">
              <input
                value={it.value ?? ""}
                placeholder="عدد"
                onChange={(e) => set({ ...it, value: e.target.value })}
                className={fieldClass()}
              />
              <input
                value={it.label ?? ""}
    ...
    case "spacer":
      return (
        <div>
          <label className={labelClass()}>ارتفاع (پیکسل)</label>
          <input
            type="number"
            min={10}
            max={400}
    ...
    default:
      return <p className="text-xs text-muted-foreground">این بلوک تنظیمات ویرایشی ندارد.</p>;
  }
}
...
export function BlockRender({ block }: { block: Block }) {
  ...
  switch (normalizeBlockType(block.type)) {
    ...
    case "paragraph":
      return (
        <p className="text-sm text-foreground/85 leading-7 px-4 py-2 whitespace-pre-wrap">
          {d.text}
        </p>
      );
    case "quote":
    ...
    case "button": {
      const base =
        "block w-[calc(100%-2rem)] mx-4 my-2 text-center rounded-2xl px-4 py-3 text-sm font-semibold transition";
      const style =
        d.style === "outline"
      ...
      return (
        <a href={d.url || "#"} className={`${base} ${style}`}>
          {d.label}
        </a>
      );
    }
      ...
      return (
        <figure className="mx-4 my-2">
          {d.url ? (
            ...
            <div className="h-32 rounded-2xl bg-muted grid place-items-center text-xs text-muted-foreground">
              تصویر
            </div>
          )}
          {d.caption && (
            <figcaption className="text-[11px] text-muted-foreground mt-1 text-center">
              {d.caption}
            </figcaption>
          )}
        </figure>
      ...
      return (
        <div className="mx-4 my-2 grid grid-cols-3 gap-1.5">
          {(d.images?.length ? d.images : ["", "", ""]).map((u: string, i: number) =>
            u ? (
              <img key={i} src={u} className="aspect-square rounded-lg object-cover" />
            ) : (
    ...
    case "music":
      return (
        <div className="mx-4 my-2 rounded-2xl bg-muted/50 p-3 flex items-center gap-3">
          ...
          <div className="text-xs text-foreground/80 truncate flex-1">
            {d.url || `پلیر ${d.provider}`}
          </div>
        </div>
      );
    case "menu":
      ...
      return (
        ...
        >
          ...
          <div className="text-xs">
            <div className="font-semibold">اینستاگرام</div>
    ...
    case "booking":
      return (
        <div className="mx-4 my-2 rounded-2xl bg-foreground text-background p-4 text-center">
          ...
          <button className="mt-2 rounded-full bg-background text-foreground px-4 py-1.5 text-xs font-semibold">
            {d.ctaLabel}
          </button>
        </div>
      );
    case "contact":
      return (
        <div className="mx-4 my-2 rounded-2xl border border-border divide-y divide-border text-sm">
          ...
          {d.whatsapp && (
            <div className="px-3 py-2 flex justify-between">
              <span className="text-muted-foreground">واتس‌اپ</span>
              <span dir="ltr">{d.whatsapp}</span>
            </div>
          )}
        </div>
      );
        ...
        <div className="mx-4 my-2 rounded-2xl border border-border divide-y divide-border">
          {(d.items ?? []).map((it: any, i: number) => (
            <details key={i} className="px-3 py-2 group">
              <summary className="text-sm font-medium cursor-pointer list-none flex justify-between">
                {it.q}
      ...
      return (
        <div className="mx-4 my-2 rounded-2xl overflow-hidden border border-border bg-muted">
          <iframe
            ...
            src={`https://www.openstreetmap.org/export/embed.html?bbox=${d.lng - 0.005}%2C${d.lat - 0.003}%2C${d.lng + 0.005}%2C${d.lat + 0.003}&marker=${d.lat}%2C${d.lng}`}
          />
    ...
    case "test":
      return (
        <div className="mx-4 my-2 rounded-2xl bg-gradient-to-br from-foreground/5 to-foreground/0 border border-border p-4 text-center">
          <div className="text-sm font-bold">{d.title}</div>
          <button className="mt-2 rounded-full bg-foreground text-background px-4 py-1.5 text-xs font-semibold">
            {d.ctaLabel}
          </button>
        </div>
      );
    ...
    case "countdown":
      return <Countdown title={d.title} target={d.target} />;
    case "file":
      return (
        <a
          href={d.url || "#"}
        ...
        >
          ...
          <div className="flex-1 text-xs">
            ...
            {d.size && <div className="text-muted-foreground">{d.size}</div>}
          </div>
        </a>
      );
    default: {
