...
import { ThemePresetsCard } from "@/components/admin/theme-presets-card";
...
function SettingsPage() {
  ...
  /** Async function specifically for preset application with toast feedback */
  function applyThemePreset(patch: Partial<NonNullable<typeof theme>>) {
    triggerSave();
    const next = { ...theme!, ...patch };
    const parsed = themeSchema.safeParse(next);
    if (!parsed.success) {
      toast.error(parsed.error.issues[0]?.message ?? "مقدار نامعتبر");
      return Promise.reject(new Error(parsed.error.issues[0]?.message ?? "مقدار نامعتبر"));
    }
  ...
  return (
    <div>
      ...
      <Card className="p-5 mb-3">
        ...
        <div className="grid gap-3 max-w-lg">
          ...
          <div>
            <Label>آواتار</Label>
            <ImageUploader
              currentImage={meta.avatar_url}
              onUpload={(u) => setMeta({ avatar_url: u })}
              onRemove={() => setMeta({ avatar_url: "" })}
              folderName="settings"
              aspect="square"
            ...
            <Input
              ...
              placeholder="یا آدرس آواتار را وارد کنید"
            />
          </div>
        </div>
      </Card>
      ...
      <ThemePresetsCard
        theme={theme}
        onApply={(patch) => applyThemePreset(patch as Partial<NonNullable<typeof theme>>)}
      />

      <Card className="p-5 mb-3">
        <h2 className="text-sm font-semibold mb-3">
          تم رنگ‌ها (فقط لندینگ و تست — پنل ادمین ثابت می‌ماند)
        </h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-w-2xl">
          <ColorField
            ...
            onChange={(v) => setTheme({ primary_color: v } as Partial<NonNullable<typeof theme>>)}
          />
          <ColorField
            label="رنگ ثانویه (برند)"
            value={theme.secondary_color}
            onChange={(v) => setTheme({ secondary_color: v } as Partial<NonNullable<typeof theme>>)}
          />
          <ColorField
            label="رنگ تاکیدی (برند)"
            value={theme.accent_color}
            onChange={(v) => setTheme({ accent_color: v } as Partial<NonNullable<typeof theme>>)}
          />
          <ColorField
            label="رنگ پس‌زمینه"
            value={theme.background_color}
            onChange={(v) =>
              setTheme({ background_color: v } as Partial<NonNullable<typeof theme>>)
            }
          />
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-w-2xl">
          <ColorField
            ...
            onChange={(v) => setTheme({ text_color: v } as Partial<NonNullable<typeof theme>>)}
          />
          <ColorField
            label="متن ثانویه"
            value={theme.text_secondary_color}
            onChange={(v) =>
              setTheme({ text_secondary_color: v } as Partial<NonNullable<typeof theme>>)
            }
          />
          <ColorField
            ...
            onChange={(v) =>
              setTheme({ text_tertiary_color: v } as Partial<NonNullable<typeof theme>>)
            }
          />
        </div>
        <div className="grid grid-cols-2 gap-3 max-w-md mt-4">
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setTheme({ border_radius: e.target.value } as Partial<NonNullable<typeof theme>>)
              }
              placeholder="0.75rem"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setTheme({ glass_opacity: parseFloat(e.target.value) } as Partial<
                  NonNullable<typeof theme>
                >)
              }
              className="w-full"
            />
...
function ColorField({
  ...
  // Debounce: only propagate color changes 400ms after the user stops interacting
  ...
  const handleChange = useCallback(
    (newValue: string) => {
      ...
      timerRef.current = setTimeout(() => onChangeRef.current(newValue), 400);
    },
    [], // stable — no dependencies, always uses latest onChange via ref
  );
