...
import { resolveProfile, defaultDbRow, PERSONALITY_TYPES } from "@/lib/personality-store";
import { usePersonalityProfiles, useUpsertPersonalityProfile } from "@/lib/cms";
import { triggerSave } from "@/lib/admin-store";

export const Route = createFileRoute("/admin/personality-types")({
  component: PersonalityTypesAdmin,
});
...
function PersonalityTypesAdmin() {
  ...
  function resetAll() {
    triggerSave();
    Promise.all(PERSONALITY_TYPES.map((t, i) => upsert.mutateAsync(defaultDbRow(t, i + 1))))
  ...
  return (
    <div>
        ...
        {PERSONALITY_TYPES.map((t) => {
          ...
          return (
            ...
            >
              {p.label}
              {isCustom(t, rows) && (
                <span className="ms-2 text-[10px] opacity-70">(تغییریافته)</span>
              )}
              {def.label !== p.label && (
                <span className="ms-1 text-[10px] opacity-60">[{def.label}]</span>
              )}
            </button>
          );
        })}
      ...
      <Card className="p-4">
        <div className="grid gap-3">
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              <Label>نام تیپ</Label>
              <Input value={profile.label} onChange={(e) => savePatch({ label: e.target.value })} />
            </div>
            <div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              <Label>نوشیدنی پیشنهادی</Label>
              <Input value={profile.drink} onChange={(e) => savePatch({ drink: e.target.value })} />
            </div>
            <div>
              <Label>بهترین جای نشستن</Label>
              <Input value={profile.spot} onChange={(e) => savePatch({ spot: e.target.value })} />
            </div>
          </div>
      ...
      <Card className="p-4 mt-4">
        ...
        >
          <h2 className="text-3xl font-extrabold" style={{ color: profile.color }}>
            {profile.label}
          </h2>
          <p className="text-sm font-semibold" style={{ color: profile.accentColor }}>
            {profile.tagline}
          </p>
          <p className="text-sm max-w-md leading-7" style={{ color: "#9a8a78" }}>
            {profile.description}
          </p>
