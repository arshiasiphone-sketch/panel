      ...
      {editing && (
        <Card className="p-4 mb-4">
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div className="sm:col-span-2">
              ...
              <ImageUploader
                ...
                onUpload={(u) => setEditing({ ...editing, image_url: u })}
          ...
          <div className="flex gap-2 mt-4">
            <PrimaryButton onClick={save} disabled={upsert.isPending}>
          ...
          {items.map((ev) => (
            <Card key={ev.id} className="overflow-hidden">
              ...
              <div className="p-3 flex flex-col gap-1">
                {ev.date_label && (
                  <div className="text-[11px] font-bold text-muted-foreground">{ev.date_label}</div>
                )}
                ...
                <div className="flex gap-1.5 mt-2">
                  <GhostButton onClick={() => setEditing(ev)} className="text-xs py-1.5">
                    ویرایش
                  </GhostButton>
                  <GhostButton
                    tone="danger"
                    onClick={() => {
                      triggerSave();
                      const previousUrl = ev.image_url ?? "";
                      remove.mutate(ev.id, {
                        onSuccess: () => {
                          if (previousUrl) deleteMediaByPublicUrl(previousUrl).catch(() => {});
