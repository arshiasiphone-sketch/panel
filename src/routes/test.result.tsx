  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span
              style={{ color: accentColor, fontSize: 10, letterSpacing: "0.22em", fontWeight: 700 }}
            >
              کافه خانه · ۱۴۰۵
            </span>
            {userName && (
              <span style={{ color: textSecondaryColor, fontSize: 10 }}>{userName}</span>
            )}
          </div>
              ...
              return (
                <div key={t} style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    <span style={{ color: textTertiaryColor, fontSize: 10, opacity: 0.7 }}>
                      {sc}
                    </span>
                  </div>
                  <div
          ...
          <span
            style={{ color: textTertiaryColor, fontSize: 9, textAlign: "center", opacity: 0.6 }}
          >
            kafekhaneh.ir
          </span>
        </div>
      ...
      {dataUrl ? (
        ...
        >
          {generating ? "در حال ساخت..." : "ساخت کارت اشتراک‌گذاری"}
        </motion.button>
      )}
    </div>
  );
...
function ResultPage() {
  ...
  const displayLabel = isHybrid
    ? tied.map((t) => profiles[t].label).join(" + ")
    : (visualProfile?.label ?? "");
  const displayTagline = isHybrid
    ? "ترکیبی از چند تیپ — قدرت تو در تنوعه"
    : (visualProfile?.tagline ?? "");
  ...
  const displayDrink = isHybrid
    ? tied.map((t) => profiles[t].drink).join(" یا ")
    : (visualProfile?.drink ?? "");
  const displaySpot = isHybrid
    ? tied.map((t) => profiles[t].spot).join(" / ")
    : (visualProfile?.spot ?? "");
      ...
      <div className="w-full max-w-md flex flex-col gap-6 relative z-10">
        ...
        >
          <motion.div
            ...
            style={{
              background: `radial-gradient(ellipse 70% 50% at 50% 0%, ${profile.color}14, transparent)`,
            }}
          ...
          >
            <h1 className="text-4xl font-extrabold" style={{ color: profile.color }}>
              {displayLabel}
            </h1>
        ...
        >
          ...
          <div className="flex flex-col gap-3">
            {[
              { label: "نوشیدنی پیشنهادی", value: displayDrink },
              { label: "بهترین جای نشستن", value: displaySpot },
            ].map(({ label, value }) => (
