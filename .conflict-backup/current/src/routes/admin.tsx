...
function AdminLayout() {
  ...
  if (loading || (user && roleLoading)) {
    return (
      <div dir="rtl" className="min-h-screen grid place-items-center text-sm text-muted-foreground">
        در حال بارگذاری...
      </div>
    );
  }
    ...
    return (
      ...
      >
        <div className="max-w-md text-center space-y-2">
          ...
          <button onClick={() => location.reload()} className="text-xs underline">
            تلاش مجدد
          </button>
        </div>
      </div>
    ...
    >
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-sm text-center space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          حساب «{email}» نقش ادمین ندارد. یک ادمین موجود می‌تواند با درج این کوئری در پایگاه داده
          برای شما دسترسی ایجاد کند:
        </p>
